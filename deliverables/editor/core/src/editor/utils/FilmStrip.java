...
/**
 * Texture class providing flipbook animation.  The class breaks up the image into regions, according the number of rows
 * and columns in the image.  It then indexes each region by frame, starting from the top-left and processing one row at
 * a time.  This is a subclass of TextureRegion, and so it keeps a rectangle region that tracks the active part of the
 * image to use for drawing.  See the API for that class to understand how a TextureRegion.  The primary advantage of
 * this class is that it can quickly compute the new region from the frame number.
 */
public class FilmStrip extends TextureRegion {
	...
	/**
	 * Creates a new filmstrip from the given texture.  The parameter size is to indicate that there are unused frames
	 * in the filmstrip.  The value size must be less than or equal to rows*cols, or this constructor will raise an
	 * error.
	 *
	 * @param texture The texture image to use
	 * @param rows    The number of rows in the filmstrip
